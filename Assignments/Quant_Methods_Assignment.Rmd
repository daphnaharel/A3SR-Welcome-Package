---
title: "Quantitative Methods Practice Assignment"
author: ""
output: pdf_document
---
...
There are only two possible values of "Male" in this dataset: 0 and 1. Therefore, one way to think about minimizing the vertical distances between each data point and the line is this: Find the mean height of female (i.e., everyone with Male=0), find the mean height of male (i.e., everyone with Male=1), and connect the two points $(0,\text{mean female height} )$ and $(1,\text{mean male height} )$.   

Now we can try to write the equation for this line in the form $Height=\beta_0+\beta_1*Male$. Using the strategy described above, we know that $(0,\text{mean female height})$ is the y-intercept of the line. Therefore $\beta_0 = \text{mean female height}$. We also know that $\beta_1$ is the slope of the line. Since we know two points on the line, we can calculate the slope as $\frac{y_2-y_1}{x_2-x_1}= \frac{(\text{mean male height} - \text{mean female height})}{(1-0)} = \text{mean male height} - \text{mean female height}$. So, $\beta_1 =\text{mean male height}-\text{mean female height}$. 
...
1. The mean male height is calculated below and saved as MM. Fill in the rest of the code to a) save the mean female height as MF and b) print the mean female height  
```{r}
# Save mean male height as MM
MM <- mean(height$Height[height$Male==1])

# Print mean male height   
MM

# Save mean female height MF
...
2. Calculate the mean difference for the two groups and plot the regression line     
  a) Save and print: MeanDiff = MM - MF   
  b) Add (0, MF) and (1,MM) to the plot from above (in yellow)   
  c) Draw a line connecting (0, MF) and (1,MM) using the abline() function  
```{r}
# Save the difference in means (MM-MF) as the variable MeanDiff: 
...
# Add (0, MF) to the plot (in yellow) (note: (1,MM) has been added for you)  
# Note: pch=16 makes the dot filled in, which makes it easier to see  
points(1, MM, col=7, pch=16)

# Draw the line Height=b_0 + b_1*Male where b_0=MF and b_1=MeanDiff  
...
3. Estimate the standard error for an independent sample T-test by hand (note: assume pooled standard deviation)  
  a) Calculate nM and nF, the number of males and females in the sample, respectively  
  b) Calculate sM and sF, the sample standard deviations for the males' heights and females' heights, respectively   
  ...
  d) Use the following equation: $SE(\bar{x_1}-\bar{x_2})=\sqrt{\frac{s_1^2}{n_1}+\frac{s_2^2}{n_2}}$ to estimate $SE(\bar{x_1}-\bar{x_2})$ where $s_1=s_2=s_{pooled}$ and $n_1=nF$, $n_2=nM$. Save the result as SEMeanDiff and print the result.    
  

```{r}
# Add code to calculate the number of females and save the result as nF
nM <- length(height$Height[height$Male==1])

# Add code to calculate sF, the sample standard deviation for females' heights  
...
4. Calculate a T-Statistic by hand to compare the null hypothesis that $MM-MF=0$ versus the alternative hypothesis that $MM-MF\ne0$
  a) Calculate $T=\frac{(\text{MeanDiff})-0}{\text{SEMeanDiff}}$ and save the result as Ttest  
  b) Use the pt function to calculate a p-value for Ttest (degrees freedom= nF+nM-2). Hint: You will want to use the parameter lower.tail=FALSE because Ttest should be $>0$ for this example, then multiply by 2 to get a 2-sided test  
...
7. Reprint the following values that you saved in previous problems: Mean females height (MF), difference in mean height for males and females (MeanDiff), standard error of this mean difference (SEMeanDiff), and the T statistic for the difference in means (Ttest). Which of these values can you find in the lm() output above?  
```{r}
# Re-print MF
...
# (hint: remember that b_0=MF and you can estimate the SE of the mean using the central limit theorem)
